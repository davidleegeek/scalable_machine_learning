...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    idxList = [OHEDict[f] for f in rawFeats].sort()
    valList = [1 for f in rawFeats]
    return SparseVector(numOHEFeats, idxList, valList)

# Calculate the number of features in sampleOHEDictManual
numSampleOHEFeats = len(sampleOHEDictManual)

# Run oneHotEnoding on sampleOne
sampleOneOHEFeat = oneHotEncoding(sampleOne, sampleOHEDictManual, len(sampleOne))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    idxList = [OHEDict.get(f) for f in rawFeats if f in OHEDict].sort()
    valList = [1 for i in range(len(idxList))]
    return SparseVector(len(OHEDict), idxList, valList)
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    hashDic = hashFunction(numBuckets, parsePoint(point), True)
    return LabeledPoint(point.split(",")[0], 
                        SparseVector(numBuckets, hashDic))
...
def computeSparsity(data, d, n):
    ...
    """
    return (data
            .map(lambda x: len(x.features.indices)/d)
